...
def handle_common_controller(response):
    ...
    for key in ["ROC temperature(Degree Celcius)", "ROC temperature(Degree Celsius)"]:
        if key in response["HwCfg"]:
            metrics["ctrl_temperature"].labels(controller_index).set(
                response["HwCfg"][key]
            )
            break
...
def handle_sas_controller(response):
    ...
    metrics["ctrl_ports"].labels(controller_index).set(
...
def handle_megaraid_controller(response):
    ...
    if "Drive Groups" in response:
        metrics["ctrl_drive_groups"].labels(controller_index).set(
            response["Drive Groups"]
        )
        metrics["ctrl_virtual_drives"].labels(controller_index).set(
...
def get_storcli_json(storcli_args, username, password, host):
    ...
    print("hello")
    storcli_cmd = (
        f"sshpass -p {password} ssh {username}@{host}'"
        + storcli_path
        + " "
        + storcli_args
        + "'"
    ...
    proc = subprocess.Popen(
        storcli_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def metrics():
    ...
    if target and target in config["targets"]:
        ...
        host = config["targets"][target]["host"]
        example_function(username, password, host)
